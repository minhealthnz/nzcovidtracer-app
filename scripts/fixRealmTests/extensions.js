...
function findOrCreatePermissionForRole(realmObject, permissions, roleName) {
  ...
  if (!realm.isInTransaction) {
    throw Error(
      "'findOrCreate' can only be called inside a write transaction.",
    );
  }
...
function addSchemaIfNeeded(schemaList, schemaObj) {
  ...
  if (
    schemaList.find(
      (obj) =>
        obj && (obj.name === name || (obj.schema && obj.schema.name === name)),
    ) === undefined
  ) {
...
module.exports = function (realmConstructor, context) {
  ...
  Object.defineProperties(
    realmConstructor.Collection.prototype,
    require("./collection-methods"),
  );
  ...
  Object.defineProperty(realmConstructor.Collection.prototype, "toJSON", {
    value: function (_, cache = new Map()) {
      return this.map((item, index) =>
        item instanceof realmConstructor.Object
          ? item.toJSON(index.toString(), cache)
          : item,
      );
    },
    ...
    value: function (_, cache = new Map()) {
      ...
      Object.keys(this)
        ...
        .forEach((key) => {
          ...
          result[key] =
            value instanceof realmConstructor.Object ||
            value instanceof realmConstructor.Collection
  ...
  Object.defineProperties(
    ...
    getOwnPropertyDescriptors({
      open(config) {
        ...
        let behavior = realmConstructor.exists(config)
        ...
        if (config.sync[behavior] !== undefined) {
          ...
          switch (type) {
            ...
            default:
              throw Error(
                `Invalid type: '${type}'. Only 'downloadBeforeOpen' and 'openImmediately' is allowed.`,
              );
          }
        }
        ...
        let openPromises = [];
        if (
          config.sync[behavior] !== undefined &&
          config.sync[behavior].timeOut !== undefined
        ) {
          ...
          if (
            config.sync[behavior] !== undefined &&
            config.sync[behavior].timeOutBehavior
          ) {
            ...
            switch (timeOutBehavior) {
              ...
              default:
                throw Error(
                  `Invalid 'timeOutBehavior': '${timeOutBehavior}'. Only 'throwException' and 'openLocal' is allowed.`,
                );
            }
          }
            ...
            new Promise((resolve, reject) => {
              setTimeout(() => {
                ...
                if (throwOnTimeOut) {
                  reject(
                    new Error(
                      `${config.sync.url} could not be downloaded in the allocated time: ${timeOut} ms.`,
                    ),
                  );
                } else {
        ...
        openPromises.push(
          new Promise((resolve, reject) => {
            asyncOpenTask = realmConstructor._asyncOpen(
              config,
              (realm, error) => {
                setTimeout(() => {
                  asyncOpenTask = null;
      ...
      openAsync(config, callback, progressCallback) {
        const message =
          "Realm.openAsync is now deprecated in favor of Realm.open. This function will be removed in future versions.";
      ...
      createTemplateObject(objectSchema) {
        ...
        for (let key in objectSchema.properties) {
          let type;
          if (
            typeof objectSchema.properties[key] === "string" ||
            objectSchema.properties[key] instanceof String
          ) {
  ...
  if (realmConstructor.Sync) {
    ...
    Object.defineProperties(
      realmConstructor.Sync.User,
      getOwnPropertyDescriptors(userMethods.static),
    );
    Object.defineProperties(
      realmConstructor.Sync.User.prototype,
      getOwnPropertyDescriptors(userMethods.instance),
    );
    ...
    Object.defineProperties(
      realmConstructor.Sync.Credentials,
      getOwnPropertyDescriptors(userMethods.credentials),
    );
    ...
    realmConstructor.automaticSyncConfiguration = function () {
      ...
      if (arguments.length === 0) {
        ...
        if (identities.length === 1) {
          ...
          new Error(
            `One and only one user should be logged in but found ${users.length} users.`,
          );
        }
    ...
    realmConstructor.Sync.Session.prototype.uploadAllLocalChanges = function (
      timeout,
    ) {
    ...
    realmConstructor.Sync.Session.prototype.downloadAllServerChanges = function (
      timeout,
    ) {
      return waitForCompletion(
        this,
    ...
    Object.setPrototypeOf(
      ...
      realmConstructor.Object.prototype,
    );
    ...
    Object.defineProperties(ResultSets.prototype, {
      objectType: {
        enumerable: true,
        get: function () {
          return this._matchesProperty.replace(
            subscriptionObjectNameRegex,
            "$2",
          );
        },
      },
        ...
        set: function (val) {
          if (typeof val === "string" || val instanceof String) {
            ...
            if (queryDescription === undefined) {
              throw new Error(
                "Updating a query must be done either using a String or a Results object.",
              );
            }
    ...
    Object.defineProperties(
      realmConstructor.prototype,
      getOwnPropertyDescriptors({
        permissions(arg) {
          if (!this._isPartialRealm) {
            throw new Error(
              "Wrong Realm type. 'permissions()' is only available for Query-based Realms.",
            );
          }
          ...
          if (arg === undefined) {
            ...
            let classPermissions = this.objects("__Class").filtered(
              `name = '${schemaName}'`,
            );
            if (classPermissions.length === 0) {
              throw Error(
                `Could not find Class-level permissions for '${schemaName}'`,
              );
            }
        ...
        subscriptions(name) {
          if (!this._isPartialRealm) {
            throw new Error(
              "Wrong Realm type. 'subscriptions()' is only available for Query-based Realms.",
            );
          }
        ...
        unsubscribe(name) {
          if (!this._isPartialRealm) {
            throw new Error(
              "Wrong Realm type. 'unsubscribe()' is only available for Query-based Realms.",
            );
          }
          if (typeof name === "string") {
            if (name !== "") {
              let named_subscriptions = this.objects("__ResultSets").filtered(
                `name == '${name}'`,
              );
